        ...
        if (interaction.options.getSubcommand() == "stats") {
            ...
            try {
                distribution = await fetch(
                    "https://api.minehut.com/network/players/distribution"
                ).then((res) => res.json())
                simple = await fetch(
                    "https://api.minehut.com/network/simple_stats"
                ).then((res) => res.json())
                homepage = await fetch(
                    "https://api.minehut.com/network/homepage_stats"
                ).then((res) => res.json())
            } catch (e) {
                return interaction.reply({
                    content:
                        "An error occured while fetching the data, try checking if minehut is online right now.",
                    ephemeral: true
                })
            }
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    },
                    {
                        ...
                        value: `**Total Players**: ${
                            distribution.javaTotal + distribution.bedrockTotal
                        }
                        Bedrock: ${distribution.bedrockTotal}
                        Java: ${distribution.javaTotal}
                        **Total Lobby Players**: ${
                            distribution.bedrockLobby + distribution.javaLobby
                        }
                        Bedrock Lobby: ${distribution.bedrockLobby}
                        Java Lobby: ${distribution.javaLobby}
                        **Total Server Players**: ${
                            distribution.bedrockPlayerServer +
                            distribution.javaPlayerServer
                        }
