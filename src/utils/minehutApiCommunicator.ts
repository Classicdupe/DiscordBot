export class MinehutApiCommunicator {
    ...
    constructor() {
        if(!process.env.MH_SESSIONID) throw new Error("MH_SESSIONID not found in .env")
        if(!process.env.MH_SESSIONTOKEN) throw new Error("MH_SESSIONTOKEN not found in .env")
        if(!process.env.SLG_TOKEN) throw new Error("SLG_TOKEN not found in .env")
